...
class Hotopay extends ModuleObject
{
	...
	public function getConfig()
	{
		if (self::$_config_cache === null)
		{
			...
			if(!isset(self::$_config_cache->admin_mailing_status)) self::$_config_cache->admin_mailing_status = ['N_ACCOUNT_PENDING','DONE','BILLING_DONE','REFUNDED']; // 관리자가 알림을 받을 주문 상태
	...
	public function checkUpdate()
	{
		...
		if(!$oDB->isColumnExists("hotopay_purchase","used_point")) return true;
	...
	public function moduleUpdate()
	{
		...
		if(!$oDB->isColumnExists("hotopay_purchase_item","subscription_srl"))
		{
			$oDB->addColumn('hotopay_purchase_item',"subscription_srl","number",20,0,true,"quantity");
		}
		...
		if(!$oDB->isColumnExists("hotopay_purchase","used_point"))
		{
			$oDB->addColumn('hotopay_purchase',"used_point","number",20,0,true,"product_original_price");
		}
					...
					case 6: // 기본 환율 정보 추가
						...
						$stmt = $oDB->prepare("
							...
							VALUES
								...
								('USD', 1, 'JPY', 138.597055, '2023-07-16 11:00:02'),
								('USD', 1, 'KRW', 1268.425214, '2023-07-16 11:00:02'),
								('USD', 1, 'USD', 1, '2023-07-16 11:00:02');
						");
						$stmt->execute();
						break;
